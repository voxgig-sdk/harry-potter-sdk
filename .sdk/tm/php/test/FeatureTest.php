...
class FtCtrl extends HarryPotterControl
{
...
class FtClient
{
    ...
    private HarryPotterUtility $utility;

    public function __construct(HarryPotterUtility $utility, array $options)
    {
    ...
    public function get_utility(): HarryPotterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HarryPotterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HarryPotterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new HarryPotterUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (HarryPotterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HarryPotterConfig::make_feature($name);
    ...
    private static function buildUrl(HarryPotterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HarryPotterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HarryPotterResponse($fetched);
        }
    ...
    private function fail(HarryPotterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HarryPotterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
