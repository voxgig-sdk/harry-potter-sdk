...
class HarryPotterMakeError
{
    public static function call(?HarryPotterContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HarryPotterContext([], null);
        }
        $op = $ctx->op ?? new HarryPotterOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HarryPotterResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HarryPotterError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HarryPotterError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof HarryPotterError) {
