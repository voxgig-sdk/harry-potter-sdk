...
module HarryPotterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HarryPotterResponse.new({})
    ...
    if fetch_err
      ...
      response = HarryPotterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HarryPotterResponse.new(fetched)
    else
