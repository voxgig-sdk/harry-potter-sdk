...
require_relative 'HarryPotter_types'
...
class HarryPotterSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = HarryPotterHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = HarryPotterHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, HarryPotterFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    HarryPotterUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = HarryPotterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = HarryPotterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = HarryPotterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = HarryPotterHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = HarryPotterError.new(
        "graphql_error", "HarryPotterSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = HarryPotterSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
