# HarryPotter Ruby SDK



The Ruby SDK for the HarryPotter API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = HarryPotterSDK.test(testopts, sdkopts)
```
...
raise a `HarryPotterError` (a `StandardError` subclass), so wrap
...
| `wand` | `Hash` | Information about the character's wand |
...
```
rb/
├── HarryPotter_sdk.rb       -- Main SDK module
...
The main module (`HarryPotter_sdk`) exports the SDK class
