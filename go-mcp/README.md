...
[MCP](https://modelcontextprotocol.io) server exposing the HarryPotter SDK as
two agent tools — `harry-potter_list` and `harry-potter_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/harry-potter-mcp)
make build

# 2. Provide credentials via the environment
export HARRY_POTTER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user harry-potter \
  -- /absolute/path/to/harry-potter-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./harry-potter-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// harry-potter_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// harry-potter_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/harry-potter-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user harry-potter \
     -- "$PWD"/dist/*/harry-potter-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `harry-potter_list` and `harry-potter_load` tools now appear
   in new sessions. Ask the agent to *"list character using harry-potter"*
   and it calls `harry-potter_list` with `{"entity":"character"}`.
...
```sh
export HARRY_POTTER_APIKEY=sk_live_xxx            # API key
export HARRY_POTTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `harry-potter_list` tool
...
### Call the `harry-potter_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `harry-potter_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `harry-potter_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HARRY_POTTER_APIKEY` | API key sent with every request. |
| `HARRY_POTTER_BASE` | Optional override of the API base URL. |
...
```sh
./harry-potter-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"harry-potter_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
