...
import (
	...
	omni "github.com/voxgig-sdk/harry-potter-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HarryPotterError); is {
				out := map[string]any{
					"name":    "HarryPotterError",
...
func omniCtx(arg any, client *sdk.HarryPotterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HarryPotterSDK); is {
			client = live
