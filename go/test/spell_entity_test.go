...
func TestSpellEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		spellRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.spell")))
...
func spellBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HARRY_POTTER_TEST_SPELL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HARRY_POTTER_TEST_SPELL_ENTID"])
