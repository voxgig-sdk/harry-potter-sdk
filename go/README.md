# HarryPotter Golang SDK



The Golang SDK for the HarryPotter API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/harry-potter-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/harry-potter-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/harry-potter-sdk/go=../harry-potter-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *HarryPotterSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### HarryPotterSDK methods
...
| `Character` | `(data map[string]any) HarryPotterEntity` | Create a Character entity instance. |
| `Spell` | `(data map[string]any) HarryPotterEntity` | Create a Spell entity instance. |

### Entity interface (HarryPotterEntity)

All entities implement the `HarryPotterEntity` interface.
...
| `length` | `float64` | Length of the wand in inches |
| `name` | `string` | Name of the character |
| `patronus` | `string` | The character's Patronus form |
| `wand` | `map[string]any` | Information about the character's wand |
