...
class TestCharacterEntity:
    ...
    def test_should_stream(self):
        ...
        base = HarryPotterSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HARRY_POTTER_TEST_CHARACTER_ENTID JSON to run live")
        client = setup["client"]
...
def _character_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("HARRY_POTTER_TEST_CHARACTER_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("HARRY_POTTER_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = HarryPotterSDK(helpers.to_map(merged_opts))

    _live = env.get("HARRY_POTTER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("HARRY_POTTER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
