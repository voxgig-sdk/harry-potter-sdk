...
class HarryPotterTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HarryPotterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HarryPotterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HarryPotterTestRunner.getenv("HARRY_POTTER_TEST_LIVE")
        override = HarryPotterTestRunner.getenv("HARRY_POTTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HarryPotterTestRunner.getenv(key)
        ...
        explain = HarryPotterTestRunner.getenv("HARRY_POTTER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if HarryPotterTestRunner._test_control is not None:
            return HarryPotterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HarryPotterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HarryPotterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HarryPotterTestRunner.load_test_control()
...
def load_env_local():
    HarryPotterTestRunner.load_env_local()


def env_override(m):
    return HarryPotterTestRunner.env_override(m)


def entity_data(v):
    return HarryPotterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HarryPotterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HarryPotterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HarryPotterTestRunner.load_test_control()


def live_client_options():
    return HarryPotterTestRunner.live_client_options()


def live_delay_ms():
    return HarryPotterTestRunner.live_delay_ms()
