...
//     (\`new HarryPotterSDK(...)\` / \`HarryPotterSDK.test(...)\` become
//     \`HarryPotterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"spell":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HarryPotterSDK(...)\` and any
// \`HarryPotterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HarryPotterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HarryPotterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
