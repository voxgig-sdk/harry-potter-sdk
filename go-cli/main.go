// Command HarryPotter-cli is a CLI + REPL for the HarryPotter SDK,
...
const prompt = "harry-potter"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "character spell"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HARRY_POTTER_APIKEY (api key), HARRY_POTTER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HARRY_POTTER_APIKEY carries the API key and
	// HARRY_POTTER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HARRY_POTTER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HARRY_POTTER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHarryPotterSDK(opts)
