...
boru-driven command-line client **and** interactive REPL for the HarryPotter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/harry-potter-cli)
make build

# 2. See usage (words, entities, env vars)
./harry-potter-cli --help

# 3. Provide credentials once, via the environment
export HARRY_POTTER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./harry-potter-cli list character
./harry-potter-cli load 1 character            # {id:1} shorthand
./harry-potter-cli load '{id:1}' character       # explicit match map
./harry-potter-cli list spell

# 5. Override the API base URL for a single call
HARRY_POTTER_BASE=https://api.example.com ./harry-potter-cli list character

# 6. No arguments -> interactive REPL
./harry-potter-cli
harry-potter> list character
harry-potter> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/harry-potter-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/harry-potter-cli list character
   ```
...
```sh
./harry-potter-cli load 1 character          # scalar shorthand for {id:1}
./harry-potter-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `harry-potter>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/harry-potter-cli`. |
...
(`./harry-potter-cli <expr>`) and interactively (the REPL), and expressions compose the
